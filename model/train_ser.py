...
from models.ser_model_efficient import Ser_Model######
...
def test(mode, params, model, criterion_ce, criterion_mml, test_dataset, batch_size, device,
    ...
    if mode == 'TEST':
        for index in range(4):
            ...
            visualize_tsne_2(str(index)+'withoutCEA', tsne, sne_labels, params)    
    
    '''#plot
    #if mode=='TEST':
    out_M = TSNE(n_components=2).fit_transform(test_outputs['F4'].cpu().numpy())
        #test_F1=test_outputs['F1']
    visualize_tsne_2('withCEA', out_M, sne_labels, params)
        #visualize_tsne_2('withCEA_WLM', test_F1, sne_labels, params)'''
...
def visualize_tsne_points_2(name, tx, ty, labels, params):
    ...
    ax.legend(loc='best', fontsize=16, markerscale=2)
    plt.show()
    
    t = round(time.time()*1000)
    t_str = time.strftime('%H_%M_%S',time.localtime(t/1000))

    img_path = './results/t-SNE/' + t_str + '_' + name + '_' + params['repeat_idx'] + '_' +'again'+ params['test_id'] + '.png'
    # finally, show the plot
    fig.savefig(img_path, dpi=fig.dpi)#原来的'''
...
'''def visualize_tsne_points_2(name, tx, ty, labels, params):
    ...
    for label in colors_per_class:
        ...
        # add a scatter plot with the correponding color and label
        fig.scatter(current_tx, current_ty, s=1, c=color, label=label)
...
def train(dataset, params, save_label='default'):
    ...
    #select device
    if params['use_gpu'] == 1:
        device = torch.device("cuda:1")########0->1
    else:
    ...
    #print('2222222222222222222222222',next(model.parameters()).device)
    print('\n')
